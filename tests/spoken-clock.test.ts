...
test("stripSpokenClockEcho removes duplicated checkpoint wording", () => {
  ...
  assert.equal(stripSpokenClockEcho("ET", "After ninety minutes, Spain and Argentina remain level."), "Spain and Argentina remain level.");
});
...
test("checkpoints and pressure ranges are narrated clearly", () => {
  ...
  assert.equal(spokenClockLead("ET"), "At the start of extra time,");
